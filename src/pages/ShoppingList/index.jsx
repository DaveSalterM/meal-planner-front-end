			...
			for (let j = 0; j < userRecipes[i][0].ingredients.length; j++) {
				...
				if (userRecipes[i][1] > 0) {
					...
					if (!shoppingList[userRecipes[i][0].ingredients[j].ingredient]) {
						...
						//Create amount by multiplying the ingredient amount by the recipe quantity
						console.log(
							userRecipes[i][0].ingredients[j].ingredient,
							userRecipes[i][1]
						);
						let newAmount =
						...
						let convertedAmount = convert(newAmount)
							.from(userRecipes[i][0].ingredients[j].unit)
							.toBest({ cutoffNumber: 2 });
						console.log(
							'converted amount for: ',
							userRecipes[i][0].ingredients[j].ingredient,
							newAmount,
							userRecipes[i][0].ingredients[j].unit,
							convertedAmount
						);
						shoppingList[userRecipes[i][0].ingredients[j].ingredient].amount +=
							convertedAmount.val;
						shoppingList[userRecipes[i][0].ingredients[j].ingredient].unit =
							convertedAmount.unit;
					}
				}
			}
		} //End of loop
